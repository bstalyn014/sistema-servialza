...
const ResidencialPage = () => {
    ...
    // Logic: Usuario Ausente disables leak detection fields
    const esUsuarioAusente = formData.prueba_consumo === 'No' && formData.prueba_razon === 'usuario ausente';

    const generarResumen = (e) => {
        e.preventDefault();
        
        let text = `ANÁLISIS DE CONSUMO RESIDENCIAL: Para el contrato ${formData.contrato}, con servicio ${formData.servicio}. `;
        ...
        // Datos específicos
        text += `Descripción de casa: ${formData.descripcion}. Personas que habitan: ${formData.personas}. `;
        text += `Cambios en habitantes (6-12 meses): ${formData.cambios_habitantes}. `;
    ...
    const guardarEnSheets = async () => {
        setIsSaving(true);
        const result = await enviarAGSheets('residencial', formData);
        setIsSaving(false);
        if (result.success) {
            setIsSaved(true);
    ...
    const resetForm = () => {
        setFormData({
            ...
            lectura_m3: '', lectura_litros: '', descripcion: '', personas: '', abastecimiento: '',
            abastecimiento_otro: '', cambios_habitantes: '', caudal_ingreso: '', caudal_cantidad: '',
            prueba_consumo: '', prueba_razon: '', motivo_no_permite: '', otra_razon: '',
            fugas_detectadas: '', puntos_agua_no: '', puntos_agua_si: '', ubicacion_fuga_detalle: '',
            persona_atendio: '', observacion: ''
        });
        setResumen(null);
        setIsSaved(false);
    };
        ...
        <div className="formulario active">
            ...
            <form onSubmit={generarResumen}>
                <div className="form-section">
                    ...
                    <FormGroup label="Descripción de casa" required>
                        <Input name="descripcion" value={formData.descripcion} onChange={handleChange} required />
                    </FormGroup>

                    <FormGroup label="¿Cuántas personas habitan en el predio?" required>
                        <Input type="number" name="personas" value={formData.personas} onChange={handleChange} required />
                    </FormGroup>
                </div>
                ...
                <div className="form-section">
                    ...
                    <FormGroup label="¿Ha habido cambios en el número de habitantes (últimos 6-12 meses)?" required>
                        <RadioGroup 
                            name="cambios_habitantes"
                            options={["Sí", "No"]}
                            value={formData.cambios_habitantes}
                            onChange={handleChange}
                        />
                    </FormGroup>
                </div>
                ...
                <div className="form-section" style={{opacity: esUsuarioAusente ? 0.5 : 1, pointerEvents: esUsuarioAusente ? 'none' : 'auto'}}>
                    ...
                    <div style={{pointerEvents: 'auto', opacity: 1}}> {/* Override generic disable for this specific field */}
